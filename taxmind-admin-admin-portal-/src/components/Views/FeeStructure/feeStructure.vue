<template>
  <div>
    ...
    <v-layout wrap justify-center>
      <v-flex pt-5 xs11 sm11 md11 lg11 xl11 pb-5>
        ...
        <v-layout wrap justify-start class="my-3">
          <v-flex xs12 sm9 md9 lg10 text-start align-center pt-2 class="carousalhead">Fee Structure</v-flex>
          <v-flex xs12 sm3 md3 lg2 text-end align-center>
            <PermissionButton v-if="canEdit('fee_structure')" module-name="fee_structure" permission="edit"
              color="#1A73E9" dark class="rounded-lg carousaladdedit" @click="editdialog = true">
              <span style="text-transform: none">EDIT FEE STRUCTURE</span>
            </PermissionButton>
          </v-flex>
        </v-layout>
    ...
    <v-dialog v-model="editdialog" max-width="800px">
      <v-card rounded="lg">
        <v-card-title>
          <span class="editbox">EDIT FEE STRUCTURE</span>
          <v-spacer></v-spacer>
  ...
  methods: {
    async getData() {
      ...
      try {
        ...
        if (success) {
          const feePolicies = (data || []).filter(
            (p) => p.type === "fee_structure"
          );
          if (feePolicies.length > 0) {
            feePolicies.sort(
              (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
            );
            this.aboutlist = feePolicies[0];
          } else {
            this.aboutlist = { content: "", type: "fee_structure" };
          }
        } else {
          this.ServerError = false;
          this.msg = message || "Failed to fetch Fee Structure";
