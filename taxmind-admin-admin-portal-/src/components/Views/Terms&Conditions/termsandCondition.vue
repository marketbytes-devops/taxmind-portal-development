<template>
  <div>
    ...
    <v-layout wrap justify-center>
      <v-flex pt-5 xs11 sm11 md11 lg11 xl11 pb-5>
        ...
        <v-layout wrap justify-start class="my-3">
          <v-flex xs12 sm9 md9 lg10 text-start align-center pt-2 class="carousalhead">Terms & Conditions</v-flex>
          <v-flex xs12 sm3 md3 lg2 text-end align-center>
            <v-btn v-if="canEdit('terms_and_conditions')" dark color="#1A73E9" @click="editdialog = true" class="rounded-lg carousaladdedit">
              <span style="text-transform: none">EDIT T&C</span>
            </v-btn>
          </v-flex>
        </v-layout>
    ...
    <v-dialog v-model="editdialog" max-width="800px">
      <v-card rounded="lg">
        <v-card-title>
          <span class="editbox">EDIT TERMS & CONDITIONS</span>
          <v-spacer></v-spacer>
        ...
        <v-card-actions class="py-4 justify-end headline grey lighten-2">
          ...
          <v-btn v-if="canEdit('terms_and_conditions')" dark color="#1A73E9" @click="validateUpdate()" class="rounded-lg carousaladdedit">
      ...
      try {
        ...
        if (success) {
          const list = (data || []).filter(
            (p) => p.type === "terms_and_condition"
          );
          if (list.length > 0) {
            list.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
            this.aboutlist = list[0];
          } else {
            this.aboutlist = { content: "", type: "terms_and_condition" };
          }
        } else {
          this.ServerError = false;
          this.msg = message || "Failed to fetch Terms & Conditions";
