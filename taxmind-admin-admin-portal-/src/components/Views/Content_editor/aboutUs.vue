    ...
    <v-layout wrap justify-center>
      <v-flex pt-5 xs11 sm11 md11 lg11 xl11 pb-5>
        ...
        <v-layout wrap justify-start class="my-3">
          <v-flex xs12 sm9 md9 lg10 text-start align-center pt-2 class="carousalhead">About us</v-flex>
          <v-flex xs12 sm3 md3 lg2 text-end align-center>
            <PermissionButton module-name="about_us" permission="edit" color="#1A73E9"
              class="rounded-lg carousaladdedit" @click="editdialog = true" dark>
              <span style="text-transform: none">EDIT ABOUT US</span>
            </PermissionButton>
          </v-flex>
        </v-layout>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      try {
        const result = await siteContentAPI.getAboutUsContent();
        ...
        this.handleError(error.message || "Failed to load About Us content");
      } finally {
    ...
    async editAbout() {
      ...
      try {
        const result = await siteContentAPI.updateAboutUsContent(
          this.aboutlist.content
        );

        if (result.success) {
          this.editdialog = false;
          this.msg = result.message || "About Us content updated successfully";
          this.showSnackBar = true;
        ...
        this.handleError(error.message || "Failed to update About Us content");
      } finally {
        this.appLoading = false;
      }
    },
    ...
    handleError(message) {
      console.error("About Us error:", message);
      this.msg = message;
      this.showSnackBar = true;
      this.appLoading = false;
      this.ServerError =
