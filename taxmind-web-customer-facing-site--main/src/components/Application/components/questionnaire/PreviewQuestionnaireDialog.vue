<template>
  <div :max-width="isMobile ? '100vw' : '1280px'" :max-height="isMobile ? '100vh' : '80vh'" :fullscreen="isMobile"
    persistent scrollable content-class="preview-dialog-overlay"
    ...
    <v-card elevation="0" class="preview-card white" :class="{ 'mobile-fullscreen': isMobile }">
      ...
      <v-layout v-if="isMobile && hasCategories" class="mobile-header" align-center justify-space-between>
        <v-btn icon @click="sidebarOpen = true" class="mobile-menu-btn">
          <v-icon color="#1A73E9">mdi-menu</v-icon>
        </v-btn>
        <div class="mobile-header-content">
          <span class="mobile-current-category">{{ activeCategory.name }}</span>
          <span class="mobile-category-indicator">{{ activeIndex + 1 }} of {{ categoriesToShow.length }}</span>
        </div>
            ...
            <v-layout v-if="hasCategories" wrap class="tile-grid">
              <v-flex xs6 v-for="(cat, i) in categoriesToShow" :key="cat.id || i" class="tile-cell">
                <v-card elevation="0" class="tile-card" :class="{
                  ...
                  disabled: cat.isDisabled,
                  completed: cat.isCompleted,
                  'has-incomplete': hasCategoryIncomplete(cat, i)
                }" @click="handleCategoryClick(cat, i)">
                  <v-layout column align-center justify-center class="tile-content">
                    <!-- Incomplete Warning Icon -->
                    <v-icon v-if="hasCategoryIncomplete(cat, i)" class="incomplete-icon" color="#FF9800"
                      size="20">mdi-alert-circle</v-icon>
                    <!-- Completed Check Mark -->
                    <v-icon v-else-if="cat.isCompleted" class="completed-icon" color="#4CAF50"
      ...
      <v-container style="padding-top: 13px !important;" fluid class="preview-footer pa-0">
        <!-- Warning Message for Incomplete Mandatory Questions -->
        <v-row v-if="!canSubmit && isLastCategory" no-gutters class="mb-2">
          <v-col cols="12">
            <v-alert dense outlined type="warning" class="incomplete-warning mb-0">
              <div class="d-flex align-center">
                <v-icon color="warning" size="20" class="mr-2">mdi-alert-circle</v-icon>
                <span class="warning-text">{{ submitWarningMessage }}</span>
              </div>
            </v-alert>
          </v-col>
        </v-row>
        <v-row no-gutters align="center" justify="space-between" class="fill-height">
          ...
          <v-col :cols="isMobile ? '12' : 'auto'" class="d-flex mx-1"
            ...
            <v-btn v-else :ripple="false" depressed color="success" @click="submitQuestionnaire" :loading="isSubmitting"
              :disabled="isSubmitting || !canSubmit" class="footer-btn submit-btn ml-3" :class="{ 'mobile-btn': isMobile }">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      validationTimestamp: Date.now(), // Force reactivity on validation checks
    };
  },
  computed: {
    categoriesToShow() {
    ...
    // Check all categories for incomplete mandatory questions
    incompleteCategoryValidation() {
      // Force reactivity by accessing validationTimestamp
      this.validationTimestamp;
      
      const incompleteCategories = [];
      
      if (!this.categories || this.categories.length === 0) {
        return { hasIncomplete: false, categories: [] };
      }

      this.categories.forEach((category, index) => {
        if (category.isDisabled || !category.questions || category.questions.length === 0) {
          return;
        }

        const hasIncomplete = this.checkCategoryForIncompleteMandatory(category.questions);
        if (hasIncomplete) {
      ...
      console.log('🔍 Validation Check:', {
        hasIncomplete: incompleteCategories.length > 0,
        incompleteCategories,
        previewResponses: this.previewResponses
      });
    ...
    canSubmit() {
      return !this.incompleteCategoryValidation.hasIncomplete;
    },
    submitWarningMessage() {
      const incomplete = this.incompleteCategoryValidation;
      if (!incomplete.hasIncomplete) {
        return '';
      }

      if (incomplete.categories.length === 1) {
        const categoryName = incomplete.categories[0].name;
        return `Please complete all required questions in "${categoryName}" before submitting.`;
      } else {
        const categoryList = incomplete.categories
          .map(cat => `"${cat.name}" (Category ${cat.index + 1})`)
          .join(', ');
        return `Please complete all required questions in: ${categoryList}`;
      }
    },
  },
  mounted() {
  ...
  watch: {
    ...
    previewResponses: {
      handler(newVal, oldVal) {
        console.log('📝 Preview responses changed:', { newVal, oldVal });
        // Use $nextTick to ensure DOM updates are complete before validation
        this.$nextTick(() => {
          this.validationTimestamp = Date.now();
          console.log('✅ Validation timestamp updated:', this.validationTimestamp);
        });
      },
      deep: true,
    },
  ...
  methods: {
    ...
    checkCategoryForIncompleteMandatory(questions) {
      ...
      for (const question of questions) {
        ...
        // Check if this is a mandatory/required question without an answer
        // Support both isRequired and isMandatory property names
        const isRequiredQuestion = question.isRequired || question.isMandatory;
        
        if (isRequiredQuestion) {
          const hasAnswer = this.checkQuestionHasAnswer(question);
          if (!hasAnswer) {
            console.warn('⚠️ Required question not answered:', {
              questionId: question.id,
              questionText: question.questionText || question.text || question.label,
        ...
        // For radio/dropdown questions, only check nested questions for the SELECTED option
        if ((question.questionType === 'radio' || question.questionType === 'dropdown') && 
            question.options && question.options.length > 0) {
          
          const selectedValue = this.previewResponses[question.id];
          
          // Only check nested questions if an option is selected
          if (selectedValue !== null && selectedValue !== undefined) {
            const selectedOption = question.options.find(opt => 
              opt.value === selectedValue || opt.label === selectedValue || opt.id === selectedValue
            );
            
            // Check nested questions only for the selected option
            if (selectedOption && selectedOption.questions && selectedOption.questions.length > 0) {
              const hasIncomplete = this.checkCategoryForIncompleteMandatory(selectedOption.questions);
              if (hasIncomplete) {
                return true;
              }
            }
        ...
        // For checkbox questions, check nested questions for ALL selected options
        else if (question.questionType === 'checkbox' && question.options && question.options.length > 0) {
          const selectedValues = this.previewResponses[question.id];
          ...
          if (Array.isArray(selectedValues) && selectedValues.length > 0) {
            for (const option of question.options) {
              ...
              if (isSelected && option.questions && option.questions.length > 0) {
                const hasIncomplete = this.checkCategoryForIncompleteMandatory(option.questions);
                if (hasIncomplete) {
                  return true;
                }
              }
        ...
        // For other question types with options/children, check all nested questions
        ...
        // Handle legacy children format
        if (question.children && question.children.length > 0) {
          const hasIncomplete = this.checkCategoryForIncompleteMandatory(question.children);
          if (hasIncomplete) {
            return true;
          }
        }
    ...
    checkQuestionHasAnswer(question) {
      // Prefer previewResponses (source of truth), fall back to live tree responses if needed
      const tree = this.$refs.questionnaireTree && this.$refs.questionnaireTree.activeResponses;
      const response = this.previewResponses?.[question.id] !== undefined
        ? this.previewResponses[question.id]
        : tree?.[question.id];

      const isDateValue = response instanceof Date;
      const isValidDate = isDateValue && !Number.isNaN(response.getTime());

      const hasAnswer = (() => {
        if (response === null || response === undefined) return false;
        if (isDateValue) return isValidDate;
        if (typeof response === 'string') return response.trim() !== '';
        if (Array.isArray(response)) return response.length > 0;
        // For numbers/booleans/objects, treat non-nullish as answered
        return true;
      })();

      console.log(`❓ Checking question ${question.id} (${question.questionText || question.text || question.label}):`, {
        isRequired: question.isRequired || question.isMandatory,
        response,
        hasAnswer
      });
    ...
    hasCategoryIncomplete(category, index) {
      const incomplete = this.incompleteCategoryValidation;
      if (!incomplete.hasIncomplete) {
        return false;
      }
      return incomplete.categories.some(cat => cat.index === index);
    },

    forceValidationUpdate() {
      // Force validation to re-run by updating the timestamp
      this.validationTimestamp = Date.now();
      console.log('🔄 Forced validation update');
    },
  },
};
</script>
...
/* Incomplete Category Warning Styles */
.tile-card.has-incomplete {
