        ...
        <v-layout wrap justify-center style="background-color: #f1f7ff">
          <v-flex xs12 pa-5>
            <v-layout wrap justify-center>
              <v-flex xs12 lg8 md7 sm6 :text-left="$vuetify.breakpoint.xs ? false : true">
                <span class="profilehead">Welcome <span class="profilename">{{ appUser }},</span> Let’s
                  start your tax refund journey.</span>
              </v-flex>
              <v-flex xs12 lg4 md5 sm6 :text-end="$vuetify.breakpoint.xs ? false : true">
    ...
    <v-layout wrap justify-center pt-lg-3 pt-sm-0 pt-md-0>
      <v-flex xs12 py-2 v-for="(file, index) in files" :key="index">
        <v-layout wrap justify-center style="background-color: #f1f7ff">
          <v-flex xs12 pa-5>
            <v-layout wrap justify-center>
              <v-flex xs10 lg8 md7 sm6 text-left>
                <span style="
                    font-family: DMSans;
                    font-weight: 700;
                    font-size: 22px;
              ...
              <v-flex xs2 lg4 md5 sm6 text-end>
            ...
            <v-layout wrap justify-center pt-4 v-if="file.isVisible">
              <v-flex xs12>
                <v-layout wrap justify-end>
                  <v-flex xs12 text-end v-if="file && file.invoiceNo">
                    <span class="invoice">Invoice Number:</span>&nbsp;<span class="invoicesub">{{ file.invoiceNo
                      }}</span>
                  </v-flex>
                  <v-flex xs12 text-end>
                    <span class="invoice"> Date:</span>&nbsp;<span class="invoicesub">{{
                      formatDateOnly(file.create_date) }}</span>
                  </v-flex>
                  <v-flex xs12 text-end>
                          ...
                          <span v-if="file && file.commissionAmount" style="
                            ...
                            {{
                              file.commissionAmount
                                ? file.commissionAmount.toFixed(2)
                                : "0.00"
                            }}</span>
                          <span v-else style="
                              font-family: DMSans;
                      ...
                      <v-layout wrap justify-center pt-4>
                        ...
                        <v-flex xs12 sm6 md6 lg6 :text-end="$vuetify.breakpoint.xs ? false : true">
                          <span style="
                            ...
                            ">{{
                              file.paymentMethod
                                ? file.paymentMethod.charAt(0).toUpperCase() +
                                file.paymentMethod.slice(1)
                                : "-"
                            }}</span>
                        </v-flex>
                      </v-layout>
  ...
  methods: {
          ...
          if (response.data.status) {
            try {
              if (
                ...
                // Fallback while migration completes: show backend message if present
    ...
    applyreview(item) {
      ...
      if (!this.rating || !this.remarks) {
        this.$snackbar.showError("Please enter rating and remarks");
        return;
      }
      this.appLoading = true;
      axios({
            ...
            try {
              if (
                ...
                this.$snackbar.showSuccess(
                  response.data.message || "Review submitted"
                );
              }
            } catch (e) {
              this.$snackbar.showSuccess(
    ...
    getData() {
      ...
      axios({
        ...
        .then((response) => {
          ...
          this.files = response.data.data.applications.map((file) => ({
            ...file,
            isVisible: false,
          }));
        })
