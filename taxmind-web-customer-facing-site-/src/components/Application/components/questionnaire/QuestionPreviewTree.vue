<template>
  <div class="question-preview-tree">
    <v-container fluid class="pa-0">
      <v-row v-for="(q, qIndex) in questions" :key="q.id || q.questionText" 
        :ref="'question-' + q.id" 
        class="question-block ma-0" :style="indentStyle" 
            ...
            <v-col cols="12" sm="6" md="4" class="pa-2">
              <v-menu v-model="dateMenus[q.id]" :close-on-content-click="false" :nudge-right="40"
                ...
                <v-date-picker :value="activeResponses[q.id]" @input="(date) => selectDate(q.id, date)"
                  no-title>
                  <v-spacer></v-spacer>
                  <v-btn text color="primary" :disabled="q.isRequired" @click="clearDate(q)">
                    Clear
                  </v-btn>
                </v-date-picker>
              </v-menu>
  ...
  methods: {
    ...
    async submitAnswer(questionId, answer, isDeselection = false) {
      ...
      try {
        ...
        await ApplicationService.submitQuestionnaireAnswer(payload);
    ...
    selectDate(questionId, date) {
      ...
      if (date) {
        ...
        // Scroll to next question after selecting date
        this.$nextTick(() => {
          this.scrollToNextQuestion(questionId);
        });
      }
    ...
    clearDate(question) {
      ...
      // Clear validation error when user clears a date
      if (this.validationErrors[questionId]) {
        this.$delete(this.validationErrors, questionId);
      }
      ...
      // Submit deselection for non-mandatory date
      this.submitAnswer(questionId, null, true);
    },
    ...
    selectOption(question, option) {
      ...
      if (next) {
        ...
        // Scroll to next question after selection
        this.$nextTick(() => {
          this.scrollToNextQuestion(question.id);
        });
    ...
    scrollToNextQuestion(currentQuestionId) {
      console.log('📜 Auto-scroll triggered for question:', currentQuestionId);
      
      // Wait for nested questions to render if any
      this.$nextTick(() => {
        setTimeout(() => {
          const currentQuestion = this.questions.find(q => q.id === currentQuestionId);
          if (!currentQuestion) {
            console.log('⚠️ Current question not found:', currentQuestionId);
            return;
          }

          // Check if current question has nested questions that just appeared
          const hasNestedQuestions = this.nestedQuestionsData[currentQuestionId] && 
                                    this.nestedQuestionsData[currentQuestionId].hasSubQuestions;
          
          if (hasNestedQuestions) {
            // Scroll to the first nested question
            console.log('⬇️ Scrolling to nested question for:', currentQuestionId);
            this.scrollToFirstNestedQuestion(currentQuestionId);
          } else {
            // Scroll to the next sibling question
            console.log('➡️ Scrolling to next sibling question after:', currentQuestionId);
            this.scrollToNextSiblingQuestion(currentQuestionId);
          }
        }, 300); // Delay to allow nested questions to render
      });
    },
    
    scrollToFirstNestedQuestion(parentQuestionId) {
      const nestedData = this.nestedQuestionsData[parentQuestionId];
      if (!nestedData || !nestedData.subQuestions || nestedData.subQuestions.length === 0) {
        return;
      }
      
      const firstNestedQuestionId = nestedData.subQuestions[0].id;
      console.log('🎯 Scrolling to first nested question:', firstNestedQuestionId);
      
      // Try to find the nested component and scroll within it
      const nestedRef = this.$refs['nested-' + parentQuestionId];
      if (nestedRef && nestedRef[0]) {
        const nestedComponent = nestedRef[0];
        const nestedQuestionElement = nestedComponent.$el.querySelector(`[data-question-id="${firstNestedQuestionId}"]`);
        
        if (nestedQuestionElement) {
          this.smoothScrollToElement(nestedQuestionElement);
        }
      }
    },
    
    scrollToNextSiblingQuestion(currentQuestionId) {
      ...
      if (currentIndex === -1 || currentIndex === this.questions.length - 1) {
        console.log('✅ No next question to scroll to (last question or not found)');
        return;
      }
      
      const nextQuestion = this.questions[currentIndex + 1];
      console.log('🎯 Next question to scroll to:', nextQuestion.id, nextQuestion.questionText);
      
      const nextQuestionRef = this.$refs['question-' + nextQuestion.id];
      if (nextQuestionRef && nextQuestionRef[0]) {
        const element = nextQuestionRef[0].$el || nextQuestionRef[0];
    ...
    smoothScrollToElement(element) {
      ...
      // Find the scrollable container (questions-scrollable-container)
      ...
      if (scrollContainer) {
        ...
        const scrollTop = scrollContainer.scrollTop + elementRect.top - containerRect.top - 20; // 20px offset from top
        
        console.log('📜 Smooth scrolling to element:', {
    ...
    findScrollableParent(element) {
      ...
      while (parent) {
        ...
        if (overflow === 'auto' || overflow === 'scroll' || overflowY === 'auto' || overflowY === 'scroll') {
          // Check if this is the questions-scrollable-container
          if (parent.classList.contains('questions-scrollable-container')) {
            return parent;
          }
        }
      ...
      return document.querySelector('.questions-scrollable-container') || window;
    },
