<template>
  <div>
    ...
    <v-layout wrap justify-center>
      <v-flex pt-5 xs11 sm11 md11 lg11 xl11 pb-5>
        ...
        <v-layout wrap justify-start class="my-3">
          <v-flex xs12 sm9 md9 lg10 text-start align-center pt-2 class="carousalhead">Cookies Policy</v-flex>
          <v-flex xs12 sm3 md3 lg2 text-end align-center>
            <PermissionButton v-if="canEdit('cookie_policy')" module-name="cookie_policy" permission="edit"
              color="#1A73E9" dark class="rounded-lg carousaladdedit" @click="editdialog = true">
              <span style="text-transform: none">EDIT COOKIES POLICY</span>
            </PermissionButton>
          </v-flex>
        </v-layout>
    ...
    <v-dialog v-model="editdialog" max-width="800px">
      <v-card rounded="lg">
        <v-card-title>
          <span class="editbox">EDIT COOKIES POLICY</span>
          <v-spacer></v-spacer>
  ...
  methods: {
    async getData() {
      ...
      try {
        ...
        if (success) {
          const cookiesPolicies = (data || []).filter(
            (p) => p.type === "cookies_policy"
          );
          if (cookiesPolicies.length > 0) {
            cookiesPolicies.sort(
              (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
            );
            this.aboutlist = cookiesPolicies[0];
          } else {
            this.aboutlist = { content: "", type: "cookies_policy" };
          }
        } else {
          this.ServerError = false;
          this.msg = message || "Failed to fetch Cookies Policy";
