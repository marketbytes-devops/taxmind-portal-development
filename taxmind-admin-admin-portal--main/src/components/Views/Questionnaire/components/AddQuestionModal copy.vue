  ...
  <v-dialog v-model="dialog" max-width="750px" persistent z-index="9999">
    <v-card elevation="0" class="add-question-modal" style="z-index: 10000">
      ...
      <div class="modal-content">
        ...
        ">
          ...
          <div :class="questionData.answerType === 'text'
            ...
            <div v-for="(option, index) in questionData.options" :key="index" class="answer-option"
              ...
              <v-text-field :value="option.text" @input="onOptionTextChange(index, $event)"
                :placeholder="getOptionPlaceholder(index)" solo flat outlined maxlength="150" counter="150" dense
                hide-details class="option-input" background-color="white" clearable />
        ...
        <div class="document-section mt-2">
          <div class="toggle-section">
          ...
          <div v-if="questionData.isConnected" class="connection-dropdowns">
            <div class="connection-row">
              <div class="form-section connection-field">
                ...
                <v-select v-model="questionData.parentQuestion" :items="parentQuestions"
                  :loading="parentQuestionsLoading" :disabled="parentQuestionsLoading" item-text="text"
                  item-value="value" solo flat outlined dense hide-details clearable class="parent-question-select"
                  :class="{ 'error--text': parentQuestionValidationMessage }" background-color="white" :placeholder="parentQuestionsLoading
                    ? 'Loading questions...'
                    : 'Select parent question'
                    " @change="onParentQuestionChange" />
              ...
              <div class="form-section connection-field">
                ...
                <v-select v-model="questionData.parentQuestionAnswer" :items="parentQuestionAnswers" :disabled="!questionData.parentQuestion ||
                  parentQuestionAnswers.length === 0
                  " item-text="text" item-value="value" solo flat outlined dense hide-details
                  class="parent-answer-select" :class="{ 'error--text': parentAnswerValidationMessage }"
                  background-color="white" :placeholder="!questionData.parentQuestion
  ...
  watch: {
    ...
    "questionData.isConnected"(newVal) {
      ...
      if (newVal) {
        ...
        this.questionData.parentQuestionAnswer = "";
      }
    },
  ...
  methods: {
    populateFormFromQuestionData(questionData) {
      ...
      try {
        ...
        if (questionData.options && Array.isArray(questionData.options)) {
          this.questionData.options = questionData.options.map((option) => ({
            text: option.label || option.value || "",
          }));

          // Populate docAnswerList with document requirements from options
          this.docAnswerList = questionData.options
            .filter(
              (option) => option.isDocumentRequired && option.documentCategoryId
            )
            .map((option) => ({
              documentCategory: option.documentCategoryId,
        ...
        // Handle parent question connection
        if (questionData.parentQuestionId) {
          this.questionData.isConnected = true;
          this.questionData.parentQuestion = questionData.parentQuestionId;
          this.questionData.parentQuestionAnswer =
      ...
      try {
        ...
        if (success && data) {
          ...
          if (data.options && Array.isArray(data.options)) {
            this.questionData.options = data.options.map((option) => ({
              text: option.label || option.value || "",
            }));

            // Populate docAnswerList with document requirements from options
            this.docAnswerList = data.options
              .filter(
                (option) =>
                  option.isDocumentRequired && option.documentCategoryId
              )
              .map((option) => ({
                documentCategory: option.documentCategoryId,
                documentAnswer: option.label || option.value,
          ...
          if (data.parentQuestionId) {
            this.questionData.isConnected = true;
            this.questionData.parentQuestion = data.parentQuestionId;
            this.questionData.parentQuestionAnswer =
            ...
            if (data.parentQuestionId) {
              await this.fetchParentQuestionOptions(data.parentQuestionId);
            }
          }
    ...
    removeOption(index) {
      ...
      if (
        ...
        // Store the option text before removal for document-answer link synchronization
        const removedOptionText = this.questionData.options[index]?.text || "";
        ...
        if (removedOptionText) {
          this.syncDocAnswerLinksWithOptionChange(removedOptionText, "");
        }
      }
    },
    onOptionTextChange(index, newValue) {
      // Store the old value for document-answer link synchronization
      const oldValue = this.questionData.options[index]?.text || "";

      // Update the option text only. Do NOT auto-remove the option when cleared
      // to preserve the input field when the user clicks the clearable icon.
      // Removal should only happen via the explicit delete button (removeOption).
      if (this.questionData.options[index]) {
        this.questionData.options[index].text = newValue;

        // Synchronize Document-Answer Links with option text changes
        this.syncDocAnswerLinksWithOptionChange(oldValue, newValue);
      }
    },
    // Synchronize Document-Answer Links when option text changes
    syncDocAnswerLinksWithOptionChange(oldValue, newValue) {
      if (!oldValue) return; // No old value to sync

      // Find any document-answer links that reference the old option text
      const matchingLinks = this.docAnswerList.filter(
        (link) => link.documentAnswer === oldValue
      );

      if (matchingLinks.length === 0) return; // No links to update

      if (!newValue || newValue.trim() === "") {
        // Option text was cleared - remove all matching document-answer links
        this.docAnswerList = this.docAnswerList.filter(
          (link) => link.documentAnswer !== oldValue
        );
        console.log(
          `🗑️ Removed ${matchingLinks.length} document-answer link(s) for cleared option: "${oldValue}"`
        );
      } else if (newValue !== oldValue) {
        // Option text was changed - update all matching document-answer links
        matchingLinks.forEach((link) => {
          link.documentAnswer = newValue;
        });
        console.log(
          `🔄 Updated ${matchingLinks.length} document-answer link(s): "${oldValue}" → "${newValue}"`
        );
      }
    },
    ...
    async saveQuestion() {
      try {
        ...
        if (
          ...
          const hasParentAnswer = this.questionData.parentQuestionAnswer !== "";
          ...
          this.questionData.answerType === "date" ||
            ...
            : this.questionData.options
              ...
              .map((opt, index) => {
                ...
                const docAnswerPair = this.docAnswerList.find(
                  (pair) => pair.documentAnswer === opt.text.trim()
                );
                const isDocumentRequiredForThisOption =
                  docAnswerPair !== undefined;
        ...
        const requestData = {
          ...
          ...(this.questionData.isConnected &&
            ...
            showIfParentOptionValue: this.questionData.parentQuestionAnswer,
          }),
        };
...
.parent-question-select .v-input__control {
  height: 40px !important;
}

.parent-answer-select .v-input__control {
  height: 40px !important;
}

.parent-question-select .v-select__selection,
.parent-answer-select .v-select__selection {
  font-family: "DM Sans", sans-serif !important;
  font-weight: 400 !important;
  font-size: 14px !important;
