        ...
        <v-layout wrap justify-center>
            ...
            <v-layout wrap justify-start>
                <v-flex xs12>
                    ...
                    <v-layout wrap justify-center>
                        <v-flex xs12>
                            <v-data-table :headers="headers" :items="paymentsList" class="elevation-0 rejected-table"
                                hide-default-footer :items-per-page="limit" :mobile-breakpoint="600"
                                ...
                                <template v-slot:item="{ item, index }">
                                    <tr style="cursor: pointer" @click="viewPaymentDetails(item)">
                                        ...
                                        <td class="body-cell">{{ item.applicationNo || '-' }}</td>
                                        <td class="body-cell">{{ item.user?.name || item.customer || '-' }}</td>
                                        <td class="body-cell">{{ formatAmount(item.finalAmount) }}</td>
                                        <td class="body-cell">{{ item.year || '-' }}</td>
                                        <!-- <td class="body-cell">{{ item.invoiceNumber || item.invoice || '-' }}</td> -->
                                        <td class="body-cell">{{ formatDate(item.createdAt || item.datetime) }}</td>
                                        <!-- <td class="body-cell">{{ formatAmount(item.claimedAmount) }}</td> -->
                                        <!-- <td class="body-cell">{{ item.rejectionCategory || item.reason || '-' }}</td>
                                        <td class="body-cell">{{ item.rejectionReason || item.reason || '-' }}</td> -->
                                        <!-- <td class="body-cell">{{ item.status || 'REJECTED' }}</td> -->
                                    </tr>
                                </template>
                            </v-data-table>
                        </v-flex>
...
<script>
import { listPendingPayments } from "@/api/modules/payments";
import debounce from "lodash/debounce";

export default {
    name: "RejectedPaymentsTable",
    props: {
        searchKeyword: {
            type: String,
            default: "",
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: "Final Amount", value: "finalAmount", align: "start" },
                { text: "Year", value: "year", align: "start" },
                // { text: "Invoice No", value: "invoice", align: "start" },
                { text: "Date & Time", value: "datetime", align: "start" },

                // { text: "Rejection Category", value: "rejectionCategory", align: "start" },

                // { text: "Rejection Reason", value: "reason", align: "start" },
    ...
    methods: {
        ...
        getData() {
            ...
            listPendingPayments(params)
                .then((response) => {
                    ...
                    console.log("Rejected payments data=", response.data);
        ...
        viewPaymentDetails(item) {
            ...
            console.log("View rejected payment details:", item);
